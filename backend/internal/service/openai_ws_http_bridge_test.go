...
func TestProxyResponsesWebSocketFromClientForGrokUsesXAIHTTPBridge(t *testing.T) {
	...
	require.Equal(t, xai.GrokCLITokenAuth, upstream.lastReq.Header.Get("X-XAI-Token-Auth"))
	require.Equal(t, xai.GrokCLIClientIdentifier, upstream.lastReq.Header.Get("x-grok-client-identifier"))
